    ...
    public class GetNpdDocCard
    {
        ...
        public void FormatHeader(XElement documentBody)
        {
            ...
            if (registrationParagraph != null)
            {
                ...
                // 2.1 - дата
                if (Regex.IsMatch(parRegText, DatePattern1, RegexOptions.IgnoreCase))
                {
                    RegDate = Convert.ToDateTime(Regex.Match(parRegText, DatePattern1, RegexOptions.IgnoreCase).Groups["date"].Value);
                }
                if (RegDate == null && Regex.IsMatch(parRegText, DatePattern2, RegexOptions.IgnoreCase))
                {
                    RegDate = Convert.ToDateTime(Regex.Match(parRegText, DatePattern2, RegexOptions.IgnoreCase).Groups["date"].Value);
                }
            ...
            // 2.1 - дата
            if (Regex.IsMatch(fullStringHeader.ToString(), DatePattern1, RegexOptions.IgnoreCase))
            {
                DocDate = Convert.ToDateTime(Regex.Match(fullStringHeader.ToString(), DatePattern1, RegexOptions.IgnoreCase).Groups["date"].Value);
            }
            if (DocDate == null && Regex.IsMatch(fullStringHeader.ToString(), DatePattern2, RegexOptions.IgnoreCase))
            {
                DocDate = Convert.ToDateTime(Regex.Match(fullStringHeader.ToString(), DatePattern2, RegexOptions.IgnoreCase).Groups["date"].Value);
            }
            ...
            if (paragraphNames.Count > 0)
            {
                ...
                {
                    // Если матч сработал, а параграф только 1 и довольно длинный
                    // то это название, его не надо оффсетить
                    if ((paragraphNames.Count > 1 && paragraphNames.Skip(1).All(p => p.Value.Trim() != "")) || paragraphNames[0].Value.Length < 280)
                    {
                        offset = 1;
                    }
                }
            ...
            do
            {
                ...
                if (acceptance.Any())
                {
                    ...
                    foreach (var elem in acceptance.SelectMany(ExtractText))
                    {
                        ...
                        if (Regex.IsMatch(value, dateTemplate))
                        {
                            var date = Convert.ToDateTime(Regex.Match(value, dateTemplate, RegexOptions.IgnoreCase).Groups["date"].Value);
                            value = Regex.Replace(value, dateTemplate, date.ToString(DateTemplate + " "));
                        }
                        else if (Regex.IsMatch(value, DatePattern2))
                        {
                            var date = Convert.ToDateTime(Regex.Match(value, DatePattern2, RegexOptions.IgnoreCase).Groups["date"].Value);
        ...
        internal void FormatGrif(XElement documentBody)
        {
            ...
            while (grifParagraphs.Any())
            {
                ...
                foreach (var elem in grifParagraphs.Where(p => p.Value.Trim() != ""))
                {
                    ...
                    {
                        string text = line;
                        if (Regex.IsMatch(text, DatePattern1Custom, RegexOptions.IgnoreCase | RegexOptions.Compiled))
                        {
                            var date = Convert.ToDateTime(Regex.Match(text, DatePattern1Custom, RegexOptions.IgnoreCase).Groups["date"].Value);
                            text = Regex.Replace(text, DatePattern1Custom, date.ToString("от " + DateTemplate + " "));
                        }
                        else if (Regex.IsMatch(text, DatePattern2, RegexOptions.IgnoreCase | RegexOptions.Compiled))
                        {
                            var date = Convert.ToDateTime(Regex.Match(text, DatePattern2, RegexOptions.IgnoreCase).Groups["date"].Value);
